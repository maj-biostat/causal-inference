...
It takes the specific example of the reported effects of obesity on mortality and gives a narrative critique of the claims from the vantage point of causal inference.
...
The end use is a policy directive that has the goal of reducing excess death attributable to obesity.
...
In all, the distribution of BMI and mortality is lower in the intervention group.
However, the mortality rate varies across the studies.
The implication is that the effect of obesity is unknown, but to inform a policy decision, simply pick the study with the greatest reduction in mortality.

An observational study is now run using population-level data on BMI, death and risk factors on lifestyle and death.
...
Above, the experimental conditions were physical activity, diet or both relative to behaviour as usual.
...
When the definition of the intervention is vague, adjustment to achieve conditional exchangeability is complicated by virtue of the vast array of factors that culd influence both obesity and death.
...
4. Statistics cannot fix problems with consistency.
5. To inform policy, it could make more sense to focus on modifiable lifestyle factors than on a particular characteristic such as BMI.
